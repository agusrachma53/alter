...
<div class="col-md-5">
    <table class="table table-striped">
        ...
        <?php
        $no = 1;
        foreach($ss -> result() as $field){
        ?>
        <tr>
            <td><?php echo $no?></td>
            <td><?php echo $field->name?></td>
            <td>
              <a href="<?php echo base_url('Admin/kategori_artikel_edit/edit/'.$field->id_cat) ?>"><button class="btn btn-primary">Edit</button></a>
              <a href="<?php echo base_url('Admin/kategori_artikel_edit/delete/'.$field->id_cat) ?>"><button class="btn btn-danger">Delete</button></a>
            </td>
        </tr>
        <?php
            $no++; //$no = $no +1
